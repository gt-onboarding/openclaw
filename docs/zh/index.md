...
* **从零全新安装：** [快速上手](/zh/start/getting-started)
* **引导式安装（推荐）：** [向导](/zh/start/wizard) (`openclaw onboard`)
...
远程访问方式：[Web 界面](/zh/web) 和 [Tailscale](/zh/gateway/tailscale)
...
* **每个主机一个 Gateway（推荐）**：它是唯一被允许持有 WhatsApp Web 会话的进程。如果你需要应急 bot 或严格隔离，请运行多个 Gateway，并为它们配置隔离的配置文件和端口；参见 [Multiple gateways](/zh/gateway/multiple-gateways)。
...
* **Canvas 主机**：在 `canvasHost.port`（默认 `18793`）上运行 HTTP 文件服务器，为节点 WebView 提供 `/__openclaw__/canvas/`；参见 [Gateway configuration](/zh/gateway/configuration)（`canvasHost`）。
...
* ⏱️ **流式传输与分块** — 块级流式输出 + Telegram 草稿流式传输细节（[/concepts/streaming](/zh/concepts/streaming)）
...
```bash
openclaw message send --target +15555550123 --message "来自 OpenClaw 的问候"
```
